	...
	sc=await Bun.udpSocket({// https://wiki.bedrock.dev/servers/raknet
		...
		socket:{
			data:async(sock,x,port,addr)=>(
				x=[...x],
				addr=await(async x=>({x,toString:(
					y=>(y=y&&y.Node.Name.match(/[^\.]+/)?.[0],_=>y?`${x} (${y})`:x)
				)(await Bun.$`tailscale whois --json ${x}`.json().catch(e=>null))}))(addr),
				x[0]==1&&magick(x.slice(9))&&addr.x.startsWith('100.')&&log(svr,`Unconnected Ping from ${addr}\n`),
